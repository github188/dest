...
api_file = [
    'json_auto/start_meeting.json',
    'json_auto/autotest.json',
    'json_auto/close_meeting.json'
    ]
...
def check_response(test_api,response):
    for key,value in test_api.items():
        if  key in response:
            if  key not in Inspection_item:     #不检测项
                if isinstance(value, list) and len(value) != 0:
                    if isinstance(test_api[key][0],dict):
                        for index in range(len(test_api[key])):
                            if key in response:
                                if len(response[key]) < len(test_api[key]):
                                    print("Missing message in",key)
                                    return False
                                else:

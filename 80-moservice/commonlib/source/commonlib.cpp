...
函 数 名： IsVidCapLE
功    能： 
算法实现： 该函数只能用于同媒体类型的比较，不同类的比较无意义
全局变量： 
参    数：  TVideoStreamCap tVidCap1
TVideoStreamCap tVidCap2
返 回 值： TRUE —— 左小于等于右，即右可以接收左的码流
----------------------------------------------------------------------
修改记录    ：
日  期		版本		修改人		走读人    修改内容
2009/3/17   4.0		周广程                  创建
=============================================================================*/
...
/*=============================================================================
函 数 名： IsDVidCapLE
功    能： 比较两个双流视频流的能力大小，不同种类的视频比较无意义，因为双流特殊性，
不对码率进行比较
算法实现： 
全局变量： 
参    数：  TDStreamCapEx tDVidCap1
TDStreamCapEx tDVidCap2
返 回 值： TRUE —— 左小于等于右，即右可以接收左的码流
----------------------------------------------------------------------
修改记录    ：
日  期		版本		修改人		走读人    修改内容
2012/4/26   1.0		    周广程                  创建
=============================================================================*/
...
参    数： u8 byResLeft [IN]
           ...
           u8 byResRight[IN]
           u8 byFpsRight[IN]
返 回 值： BOOL32   ——  MbpsLeft <= MbpsRight，返回TRUE
----------------------------------------------------------------------
修改记录    ：
日  期		版本		修改人		走读人    修改内容
2009/3/13   4.0		    周广程                  创建
=============================================================================*/
...
参    数： TNetAdaptInfoAll * ptNetAdaptInfoAll
			BOOL32 bOnlyEthBegin -- TRUE 表示支持网卡名仅以Eth开头
									FALSE 表示网卡名可以为任意数字大小写字符组合
			BOOL32 bCheckIPInCfg -- TRUE 检测IP是否能够在对应的网络配置文件中找到
									FALSE 不检测
返 回 值： BOOL32 
----------------------------------------------------------------------
修改记录    ：
...
2013/1/31   2.5.2		heyisen                 增加支持任意数字字母组成的网卡名
=============================================================================*/
...
//     s32 nEthernetAdapterNum = 0;
    ...
    for ( ; tIfc.ifc_len - nCount >= sizeof(struct ifreq) ; nCount += sizeof(struct ifreq),  ptIfr += 1)
    {
		...
		{ 
			...
			//获取当网卡多ip情况的序列号，如果网卡名木有冒号，则默认为序列号为0
...
参    数： char *pKeyName
			s32 nSemId
注    意： 
返 回 值： BOOL32  创建或获取是否成功
----------------------------------------------------------------------
修改记录    ：
日  期		版本		修改人		走读人    修改内容
2014/3/14   4.8		    周晶晶                创建
=============================================================================*/
...
/*=============================================================================
函 数 名： IsLastEnterCriticalByPid
功    能： 判断信号量最后一次进入临界区是否nPid指定的进程
算法实现： 该函数和GiveSystemVSemphare函数不能在同时在多线程中使用
全局变量： 
参    数： 
			s32 nSemId
注    意： 
...
/*=============================================================================
函 数 名： GiveSystemVSemphare
功    能： 释放systemV信号量
算法实现： 该函数和IsLastEnterCriticalByPid函数不能在同时在多线程中使用
全局变量： 
参    数： 
			s32 nSemId
注    意： 
...
/*=============================================================================
函 数 名： CreateOrGetSystemVShareMemory
功    能： 创建systemV共享内存
算法实现： 
全局变量： 
参    数： char *pKeyName
...
/*=============================================================================
函 数 名： InitializeSystemVShareMemory
功    能： 初始化systemV共享内存
算法实现： 初始化全0
全局变量： 
参    数： char *pKeyName
...
/*=============================================================================
函 数 名： DeleteSystemVShareMemory
功    能： 删除systemV共享内存
算法实现： 
全局变量： 
参    数： 	s32 nShMemId
...
/*=============================================================================
函 数 名： BindCpu
功    能： 给上层业务线程调用的绑定第1和第2个cpu，现在在主线程中调用,其createapp创建的线程会自动继承
算法实现： 
全局变量： 
参    数： u8 byCpuNum：需要绑定的cpu数目
注    意： 
返 回 值： void
----------------------------------------------------------------------
修改记录    ：
日  期		版本		修改人		走读人    修改内容
2014/12/16  4.8		    周晶晶                创建
=============================================================================*/
...
/*=============================================================================
函 数 名： PrintCpuBindInfo
功    能： 打印线程所绑定的cpu信息，需要在线程内部调用
算法实现： 
全局变量： 
参    数： 	void
注    意： 

...
public:
	...
	void ToSimpleConfInfo(TSimpleConfInfo& tSimpleConfInfo) const
	{
		...
		tSimpleConfInfo.SetConfMode(GetVideoMode(GetBitRate(), GetResolution()));
	...
	void ToConfDetailInfo(TConfDetailInfo& tDetailConf)
	{
		...
		tDetailConf.SetConfMode(GetVideoMode(m_wBitRate, m_byResolution));
...
public:
	...
	LPCSTR GetCmuName( void ) const
	{
		return m_tInfo.achMcuName;
	}
	...
	LPCSTR GetGkPreFix( void ) const
	{
		return m_tInfo.achGKPreFix;
	}
	...
	void Print( void ) const
	{
		...
		OspPrintf(TRUE, FALSE, "Mcu Type:%s, Name:%s, MaxConf:%d.\n", GetCmuType(), GetCmuName(), GetMaxConfNum());
	}
};

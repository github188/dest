...
BOOL32 GetConfInfoFromJsonObj(const json_t_ptr pjRoot, TSimpleConfInfo& tConfInfo)
{
	...
	const u8 byConfMode = GetVideoMode((u16)dwConfBitrate, (u8)dwResolution);
...
BOOL32 GetConfInfoFromJson(const json_t_ptr pjRoot, TConfDetailInfo& tConfInfo)
{
	...
	const u8 byConfMode = GetVideoMode((u16)dwConfBitrate, (u8)dwConfResolution);
	tConfInfo.SetConfMode(byConfMode);
	u8 byCMMode = (dwOpenMode == 0 ? CONF_OPENMODE_CLOSED : CONF_OPENMODE_OPEN);
	tConfInfo.SetOpenMode(byCMMode);
...
void ConvertStruct(const CRedisBookConf& cBookConf, TSimpleConfInfo& tSimpleConf)
{
	...
	tSimpleConf.SetConfMode(GetVideoMode(cBookConf.GetBitRate(), cBookConf.GetResolution()));
...
void ConvertStruct(const CRedisBookConf& cBookConf, TConfDetailInfo& tDetailConf)
{
	...
	tDetailConf.SetConfMode(GetVideoMode(cBookConf.GetBitRate(), cBookConf.GetResolution()));
...
BOOL32 GetRedisConfFromJsonObj(const json_t_ptr& pjRoot, TRedisConfInfo &tConfInfo)
{
	...
	tConfInfo.SetDomianMOID(pchMoid);
...
BOOL32 GetRedisBookConfFromJsonObj(const json_t_ptr& pjRoot, CRedisBookConf &cBookConf)
{
	...
	cBookConf.SetDomianMOID(pchDomainMOID);
...
BOOL32 SetChargeInfoToJson(const TRedisConfInfo& tRedisConf, std::string& chargeJson)
{
	...
	SetUIntAsStringToJson(pjRoot, "startTime", tBookStartTime);
	SetUIntAsStringToJson(pjRoot, "endTime", tBookStartTime);
	SetUIntAsStringToJson(pjRoot, "realStartTime", tRealStartTime);
	SetUIntAsStringToJson(pjRoot, "realEndTime", tRealEndTime);

	u32 dwJsonBufLen = 0;
	s8 achJsonBuf[10240 + 1] = { 0 };
	DumpJson(pjRoot, achJsonBuf, MAXLEN_JSONBUF, dwJsonBufLen);
...
size_t b64_encode(const char* in, size_t insize, char* out)
{
...
size_t b64_decode(const char* in, size_t insize, char* out)
{
...
s32 B64DecodeAndUnCompressXmlMsg(s8* pchData, u8* pchXmlMsg, u32& dwXmlMsgLen)
{
	...
	const u32 dwCompressXmlMsgLen = b64_decode(pchData, strlen(pchData), (s8*)abyCompressXmlMsg);
	...
	CPlatformConfNumLimit& cConfNumLimit, const BOOL32 bBmcLicense /*=TRUE*/)
{
	if (NULL == pchJsonBuf)
	{
		OspPrintf( TRUE, FALSE, "[GetMeetingLimitFromJson] Param is null return!\n" );
		return FALSE;
	}
	...
	const s8* pchJson_192_1080P_ConfNumLimit = (bBmcLicense ? BMCLICENSE_192_1080P_CONFNUM_LIMIT : BMC_192_1080P_CONFNUM_LIMIT);
	...
	const s8* pchJson_8_1080P_ConfNumLimit = (bBmcLicense ? BMCLICENSE_8_1080P_CONFNUM_LIMIT : BMC_8_1080P_CONFNUM_LIMIT);
...
BOOL32 GetCreateConfParamFromJsonObj(const json_t_ptr& pjRoot, TConfInfo& tConfInfo)
{
	...
	else
	{
		...
		OspPrintf(TRUE, FALSE, "[%s] json_object_get maxjoinedmt is error, set default: 192\n", __FUNCTION__);
	}
...
BOOL32 CheckQComsumerStatus(const s8* pchJsonBuf, const u32 dwJsonLen)
{
	...
	if (pchJsonBuf != NULL && strncmp(pchJsonBuf, "testQ", 5) == 0)
	{
		...
		time_t time = (time_t)strtoul(strCurTime.c_str(), 0, 10);
		if ((time - timeRec) == 1)
		{
...
BOOL32 SetCreatorMOIDToJson(s8 * pchJsonBuf, u32 &dwJsonBufLen, const s8 * pchCreatorMOID)
{
	...
	ReturnIfFailed(SetStringToJson(pjRoot, JSON_CREATORMOID,  pchCreatorMOID));

...
void ProducerSuccCB( amqp_connection_state_t conn, char *desc )
{	
	LogPrint( LOG_LVL_KEYSTATUS, MID_MAU_PUBLIC, "[%s]Tag:%s, %s.\n", 
		__FUNCTION__, amqp_get_tag(conn), desc);
}

void ProducerFailCB(amqp_connection_state_t conn, const char *expect, const char *precv)
{
	LogPrint(LOG_LVL_ERROR, MID_MAU_PUBLIC, "[%s]Tag:%s, expect.%s,precv.%s\n",
		__FUNCTION__, amqp_get_tag(conn), expect, precv);
}

void ConsumerSuccCB(amqp_connection_state_t conn, char *desc, char* pchTag)
{
	LogPrint( LOG_LVL_KEYSTATUS, MID_MAU_PUBLIC, "[%s]Tag:%s, %s, %s.\n",
		__FUNCTION__, amqp_get_tag(conn), desc);	
}
void ConsumerFailCB(amqp_connection_state_t conn, const char *expect, const char *precv, char* pchTag)
{
	LogPrint(LOG_LVL_ERROR, MID_MAU_PUBLIC, "[%s]Tag:%s, expect.%s,precv.%s\n", 
		__FUNCTION__, amqp_get_tag(conn), expect, precv);
}

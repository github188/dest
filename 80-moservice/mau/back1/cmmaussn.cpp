...
{
	// 创建生产者
	s_nCMMauRabbitMq = SafeCreateP( ProducerSuccCB, ProducerFailCB );
	...
	SafeCreateC( MQ_MAU_CMMAU_Q, MQ_MAU_CMMAU_EX, MQ_MAU_CMMAU_K, ContentBody_MQCB, ContenHeaderProps_MQCB, NULL,
		ConsumerSuccCB, ConsumerFailCB, 0);
...
void CCMMauSsnInstance::DaemonInstanceEntry( CMessage * const pcMsg, CApp *pcApp )
{
	...
	case MAU_CM_MCULIST_NTF:
		DaemonProcMcuListNtf( pcMsg );
		break;
...
void CCMMauSsnInstance::DaemonProcMcuListNtf( CMessage * const pcMsg )
{
	s8 achJsonBuf[MAXLEN_JSONBUF+1] = {0};	
	u32 dwJsonBufLen = pcMsg->length;
	memcpy(achJsonBuf, pcMsg->content, dwJsonBufLen);


	LogPrint( LOG_LVL_KEYSTATUS, MID_MAU_CMMAU, "[DaemonProcMcuListNtf]Send to cm,len is %d!\n", dwJsonBufLen);
	LongLogPrint(LOG_LVL_KEYSTATUS, MID_MAU_CMMAU, achJsonBuf, dwJsonBufLen);
	LogPrint(LOG_LVL_KEYSTATUS, MID_MAU_CMMAU, "[DaemonProcMcuListNtf]end\n");

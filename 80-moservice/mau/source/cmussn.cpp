...
{
	// 创建生产者
	s_nCmuRabbitMq = SafeCreateP( Producer_ConSuc_MQCB, Producer_ConFail_MQCB );
	if ( 0 == s_nCmuRabbitMq )
	{
	...
	// 创建消费者
	SafeCreateC( MQ_MAU_CONF_Q, MQ_MAU_CONF_EX, MQ_MAU_CONF_K, Content_MQCB,
		NULL, NULL, Consumer_ConSuc_MQCB, Consumer_ConFail_MQCB, 0, 0, 0, 1, g_cMauVCApp.GetMqConsumerHeartBeatTime());
...
=============================================================================*/
void CCmuSsnData::Producer_ConSuc_MQCB( amqp_connection_state_t conn, char *desc )
{	
	g_cMauVCApp.GetMqStatus().SetCmuPIsCreated(TRUE);
	...
	LogPrint( LOG_LVL_KEYSTATUS, MID_MAU_CMU, "[CCmuSsnData::Producer_ConSuc_MQCB]%s.\n", achSucDec );
...
2015/03/09  5.0         rnn                   创建
=============================================================================*/
void CCmuSsnData::Producer_ConFail_MQCB( amqp_connection_state_t conn, const char *expect, const char *precv )
{
	g_cMauVCApp.GetMqStatus().SetCmuPIsCreated(FALSE);
	...
	LogPrint( LOG_LVL_ERROR, MID_MAU_CMU, "[CCmuSsnData::Producer_ConFail_MQCB]expect.%s.\n", achExpect );
	LogPrint( LOG_LVL_ERROR, MID_MAU_CMU, "[CCmuSsnData::Producer_ConFail_MQCB]precv.%s.\n", achRecv );
}
...
2015/03/09  5.0         rnn                   创建
=============================================================================*/
void CCmuSsnData::Consumer_ConSuc_MQCB( amqp_connection_state_t conn, char *desc, char* pchTag )
{	
	g_cMauVCApp.GetMqStatus().SetMau_ConfIsCreated(TRUE);
	...
	LogPrint( LOG_LVL_KEYSTATUS, MID_MAU_CMU, "[CCmuSsnData::Consumer_ConSuc_MQCB]%s.\n", achSucDec );	
    return;
}
...
2015/03/09  5.0         rnn                   创建
=============================================================================*/
void CCmuSsnData::Consumer_ConFail_MQCB( amqp_connection_state_t conn, const char *expect, const char *precv, char* pchTag )
{	
	g_cMauVCApp.GetMqStatus().SetMau_ConfIsCreated(FALSE);
	...
	LogPrint( LOG_LVL_ERROR, MID_MAU_CMU, "[CCmuSsnData::Consumer_ConFail_MQCB]expect.%s.\n", achExpect );
	LogPrint( LOG_LVL_ERROR, MID_MAU_CMU, "[CCmuSsnData::Consumer_ConFail_MQCB]precv.%s.\n", achRecv );
}
...
void CCMUSsnInstance::DaemonProcCmuInfoNtf( CMessage * const pcMsg, CApp* pcApp )
{
	...
	for ( nCmuIdx = 0; nCmuIdx < nCmuNum; nCmuIdx++ )
	{
		...
		LogPrint( LOG_LVL_WARNING, MID_MAU_CMU, "[CCMUSsnInstance::DaemonProcCmuInfoNtf]Assign inst<Id.%d> for CMU<QName.%s> McuIp<%s> MucType<%s>\n", 
				wIdleInstID, ptCmuInfo->achRoutingKey, ptCmuInfo->achMcuIP, ptCmuInfo->achMcuType);

...
//MCU会议开始自动录像定时器，
OSPEVENT( MCUVC_CONFCALLMT_TIMER,             EV_MCU_BGN + 319 ); 
...
//mcu连接本级资源池定时器
OSPEVENT( MCURESPOOL_CONNAPLZK_TIMER,           EV_MCU_BGN + 986  );

//mcu连接本级资源池定时器
OSPEVENT( MCURESPOOL_CONNREGZK_TIMER,           EV_MCU_BGN + 987 );
...
//achConfE164 + u8 byMemberType + u8 byMode + TTransportAddr tRtpRcvAddr + TMediaEncrypt tMediaEncrypt + TDoublePayload tDoublePayload + 
// TAudAACCap tAudAACCap + TMtIdentifyToCM SrctMt + u8 bySrcChl + TTransportAddr tSrcRtcpAddr
...
//会议停止监控通知, 消息体:
// u8 byMode + TTransportAddr tRtpRcvAddr
...
//获取会议列表
OSPEVENT( MAU_MCU_GETCONFLIST_CMD,					EV_MCU_BGN + 1411);
//添加终端失败通知
OSPEVENT( CMU_CM_ADDMT_FAIL_NTF,					EV_MCU_BGN + 1412);

//通知会管终端基本信息
OSPEVENT( CMU_CM_MTINFO_NTF,						EV_MCU_BGN + 1413);
OSPEVENT( CMU_CM_MTCHNINFO_NTF,						EV_MCU_BGN + 1414);

//恢复会议
OSPEVENT( MAU_MCU_RESTORECONF_REQ,               EV_MCU_BGN + 1415 );
OSPEVENT( MCU_MAU_RESTORECONF_ACK,               EV_MCU_BGN + 1416 );
OSPEVENT( MCU_MAU_RESTORECONF_NACK,              EV_MCU_BGN + 1417 );
// 用以通知cmdp清除会议信息 [2016/5/4/wsk]
OSPEVENT( MCU_CMDP_CLEARCONF_NTF,				 EV_MCU_BGN + 1418 );
// 兼容消息，老mcu创会兼容模块发给cmdp写入url，url提供给会管用以判断是否为兼容mcu的会议 [2016/5/25/wsk]
// s8 achCmuIP[20] + u32 dwCmuPid + 5.0Tconfinfo + if(tConfInfo.HasConfExInfo()==1)TConfInfoEx + s8 url[128]
OSPEVENT( OLD_MCU_CMDP_CONFINFO_NTF,				 EV_MCU_BGN + 1419 );
// 双流源通知：  TMtIdentifyToCM DSSrc [2016/5/31/wsk]
OSPEVENT( MCU_CMDP_DSTREAMSRC_NTF,					 EV_MCU_BGN + 1420 );

//开启监控失败通知
OSPEVENT( CMU_CM_STARTMONITORINGFAIL_NTF,				EV_MCU_BGN + 1421 );
//获取终端终端实时信息:丢包率
OSPEVENT( CM_CMU_GETREALTIMEMTSTATUS_CMD,       EV_MCU_BGN + 1422 );
OSPEVENT( CMU_CM_GETREALTIMEMTSTATUS_NTF,       EV_MCU_BGN + 1423 );
//监控成功后，被监控的终端无视频源，给会管的提示
OSPEVENT( CMU_CM_STARTMONITORINGALARM_NTF,       EV_MCU_BGN + 1424 );

//css通知mcu刷新所有会议数据到redis
OSPEVENT( MAU_MCU_UPDATEALLCONFDATA_CMD,           EV_MCU_BGN + 1425 );

//主席选看非广播画面合成
OSPEVENT( CM_CMU_STARTSEEVMP_REQ,                 EV_MCU_BGN + 1426 );
OSPEVENT( CMU_CM_STARTSEEVMP_ACK,                 EV_MCU_BGN + 1427 );
OSPEVENT( CMU_CM_STARTSEEVMP_NACK,                EV_MCU_BGN + 1428 );

//停主席选看非广播的合成REQ 
OSPEVENT( CM_CMU_STOPSEEVMP_REQ,                  EV_MCU_BGN + 1429 );
OSPEVENT( CMU_CM_STOPSEEVMP_ACK,                  EV_MCU_BGN + 1430 );
OSPEVENT( CMU_CM_STOPSEEVMP_NACK,                 EV_MCU_BGN + 1431 );

//会管开始监控混音  TReqHeadInfo + TTransportAddr + TAudioTypeDesc
OSPEVENT( CM_CMU_STARTMONITORING_MIXER_REQ,			EV_MCU_BGN + 1432 );
OSPEVENT( CMU_CM_STARTMONITORING_MIXER_ACK,			EV_MCU_BGN + 1433 );
OSPEVENT( CMU_CM_STARTMONITORING_MIXER_NACK,		EV_MCU_BGN + 1434 );

//会议中级联的Mcu媒体源通知,消息体：  TMcu+TMt
OSPEVENT( CMU_CM_MCUMEDIASRC_NTF,					EV_MCU_BGN + 1435 );

//会管开始监控合成
OSPEVENT( CM_CMU_STARTMONITORING_VMP_REQ,		EV_MCU_BGN + 1436 );
OSPEVENT( CMU_CM_STARTMONITORING_VMP_ACK,		EV_MCU_BGN + 1437 );
OSPEVENT( CMU_CM_STARTMONITORING_VMP_NACK,		EV_MCU_BGN + 1438 );

//  [5/2/2017 xiayu]会馆指定终端视频源
// 会馆查询终端视频源,消息体：TOperObjToCM
OSPEVENT( CM_CMU_GETMTVIDEOSOURCEINFO_CMD,		EV_MCU_BGN + 1442 );
//上报终端视频源信息,消息体：TMtIdentifyToCM + u8 ChnNum + ChnNum*(u8 byChnIdx + u8 bySrcNum + bySrcNum*tagITVideoSourceInfo)
OSPEVENT( CMU_CM_MTALLVIDEOSOURCEINFO_NTF,		EV_MCU_BGN + 1443 );
// 会馆设置终端视频源，消息体：TMt(目的终端)+u8(FeccParam)+u8(bychannel)
OSPEVENT( CM_CMU_SETMTVIDEOSOURCE_CMD,		EV_MCU_BGN + 1444 );

//暂时API不支持同等级会议上报是否允许强拆交互
// yjj[2017/1/4]向会管请求是否强拆
// OSPEVENT( CMU_CM_RELEASEMT_REQ,                     EV_MCU_BGN + 1439 );
// OSPEVENT( CM_CMU_RELEASEMT_ACK,                     EV_MCU_BGN + 1440 );
// OSPEVENT( CM_CMU_RELEASEMT_NACK,                    EV_MCU_BGN + 1441 );
...
//worker掉线，停止广播bas相关业务  消息体：pairid
OSPEVENT( MCU_INTERACT_STOPBAS_CMD,       EV_REC_BGN + 45 );

//MCU侧,录放像定时器 共16个,预留16个  。录播没有按时成功，删除录播及相关vc侧数据,
OSPEVENT( MCUVC_MC_RECORD_TIMER,       EV_REC_BGN + 46 );
// vrs 占用 16个定时器，之后EV_REC_BGN + 62 开始
...
呼叫挂断相关
------------*/
//添加终端，	VCApp->MtMgrApp，	消息体：u8(num) + u8(addmode) +TAddMtInfo[] +u8(forcecall) + u8(是否要立即呼叫) + u8(是否是开会是呼叫)
OSPEVENT(VC_MTMGR_ADDMT_REQ,					EV_MIXER_BGN + 4 );
//添加终端，	VCApp->MtMgrApp，	消息体：u8(num) + u8(addmode) +TAddMtInfo[] +u8(forcecall) + u8(是否要立即呼叫) + u8(是否是开会是呼叫) + num*sizeof(TMT)
//											没有添加成功的终端用空的TMt填充
...
OSPEVENT(VC_MTMGR_RESTOREMTINFO_REQ,						EV_MIXER_BGN + 61 );
OSPEVENT(MTMGR_VC_RESTOREMTINFO_ACK,						EV_MIXER_BGN + 62 );
OSPEVENT(MTMGR_VC_RESTOREMTINFO_NACK,						EV_MIXER_BGN + 63 );

// vc的广播消息，先单发到mtmgr在群发给mtadpssn； body：CSermsg(记得seteventid)
OSPEVENT(VC_MTMGR_BROADCASTMSG_CMD,						EV_MIXER_BGN + 64 );
// 外厂商终端绕道mtmger翻状态，完成后再发回vc处理回环交换
OSPEVENT(VC_MTMGR_MTLOOPON_CMD,							EV_MIXER_BGN + 65 );
OSPEVENT(VC_MTMGR_MTLOOPOFF_CMD,						EV_MIXER_BGN + 66 );

//简单级联会议能力通知后更新数据
OSPEVENT(MTMGR_VC_MCUTYPE_NTF,						EV_MIXER_BGN + 67 );
// 监控占领会议四种音频编码格式
OSPEVENT(VC_MTMGR_OCCUPYMC_AUDCAP_REQ,						EV_MIXER_BGN + 68 );
OSPEVENT(MTMGR_VC_OCCUPYMC_AUDCAP_ACK,						EV_MIXER_BGN + 69 );
OSPEVENT(MTMGR_VC_OCCUPYMC_AUDCAP_NACK,						EV_MIXER_BGN + 70 );
// 释放占领的会议音频编码格式
OSPEVENT(VC_MTMGR_RELEASE_AUDCAP_CMD,						EV_MIXER_BGN + 71 );
...
/********** InterAct与InterAct内部交互信令((30501-30600)) ---复用HDU内部业务消息 bgn********/
//client状态变化通知
OSPEVENT( DSMGR_CLIENTSTATCHANGE_NTF,				EV_HDU_BGN);
//初始化，申请clientid
//消息体：会议E164
OSPEVENT( VC_DSMGR_INIT_CMD,					EV_HDU_BGN + 1);
//结会释放clientid
//消息体：无
OSPEVENT( INTER_DSMGR_RELEASE_CMD,				EV_HDU_BGN + 2 );
//client上线通知
//消息体：无
OSPEVENT( DSMGR_VC_CLIENT_CONNECT_NTF,			EV_HDU_BGN + 3 );
//client掉线通知
//消息体：无
OSPEVENT( DSMGR_VC_CLIENT_DISCONNECT_NTF,		EV_HDU_BGN + 4 );
//worker掉线后，dsmgr立马为会议申请端口，申请成功后，通知inter刷新vmp，bas，mix（n模式）的rtp地址
//消息体：workerip
OSPEVENT( DSMGR_INTER_REFRESH_ADDR_NTF,	    	EV_HDU_BGN + 5 );
//worker掉线后，dsmgr立马为会议申请端口，申请失败后，通知vc，清理业务，通知界面异常提示
//消息体：无
OSPEVENT( DSMGR_VC_WORKER_DISCONNECT_NTF,		EV_HDU_BGN + 6 );
//worker掉线 
//消息体：workerhandle
OSPEVENT( DSMGR_WORKER_DISCONNECT_NTF,	    	EV_HDU_BGN + 7 );
//worker上线
OSPEVENT( DSMGR_WORKER_CONNECT_NTF,	    	    EV_HDU_BGN + 8 );
//建交换回复
OSPEVENT( DSMGR_VC_ADDSWITCH_ACK,	    		EV_HDU_BGN + 9 );
OSPEVENT( DSMGR_VC_ADDSWITCH_NACK,	    		EV_HDU_BGN + 10 );
OSPEVENT( DSMGR_VC_ADDM2ONESWITCH_ACK,			EV_HDU_BGN + 11 );
OSPEVENT( DSMGR_VC_ADDM2ONESWITCH_NACK,			EV_HDU_BGN + 12 );
OSPEVENT( DSMGR_VC_ADDBRIDGE_ACK,	    		EV_HDU_BGN + 13 );
OSPEVENT( DSMGR_VC_ADDBRIDGE_NACK,	    		EV_HDU_BGN + 14 );
OSPEVENT( DSMGR_VC_SENDTOBRIDGE_RSP_ACK,	    EV_HDU_BGN + 15 );
OSPEVENT( DSMGR_VC_SENDTOBRIDGE_RSP_NACK,	    EV_HDU_BGN + 16 );

//rmv统一都走消息，不直接调用接口
// OSPEVENT( DSMGR_REMOVESWITCH_CMD,		    	EV_HDU_BGN + 15 );
OSPEVENT( DSMGR_VC_REMOVESWITCH_NTF,	    	EV_HDU_BGN + 17 );
// OSPEVENT( INTER_DSMGR_REMOVEM2ONESWITCH_CMD,	EV_HDU_BGN + 17 );
OSPEVENT( DSMGR_VC_REMOVEM2ONESWITCH_NTF,	    EV_HDU_BGN + 18 );
/********** InterAct与InterAct内部交互信令((30501-30600)) ---复用HDU内部业务消息 bgn********/

...
void Consumer_ConSuc_MQCB( amqp_connection_state_t conn, char *desc, char* pchTag )
{
	...
	LogPrint(LOG_LVL_KEYSTATUS, MID_ENTRANCE_APS, "[%s]%s\n", __FUNCTION__, achSucDec);
}
...
void Consumer_ConFail_MQCB( amqp_connection_state_t conn, const char *expect, const char *precv, char* pchTag )
{
	...
	LogPrint(LOG_LVL_KEYSTATUS, MID_ENTRANCE_APS, "[%s]expect.%s\n", __FUNCTION__, achExpect);
	LogPrint(LOG_LVL_KEYSTATUS, MID_ENTRANCE_APS, "[%s]recv.%s\n", __FUNCTION__, achRecv);
}
...
BOOL32 InitRBMQ( CMqConfig &tMqConf,BOOL32 bIsNeedInit)
{
    ...
    strQueueFullName += ":";
	strQueueFullName += g_cCfgFile.GetApsGUID();
	...
	nRet = SafeCreateC( tMqConf.GetQueueName(), tMqConf.GetExchangeName(), 
					tMqConf.GetBindingkey(),
					Content_MQCB, NULL, NULL, Consumer_ConSuc_MQCB, Consumer_ConFail_MQCB);	
...
// #endif
	...
	g_cCfgFile.SetGUID(GetSrvGUID().GetNOString());
#endif
#endif

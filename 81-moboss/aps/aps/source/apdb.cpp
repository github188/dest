...
u16 CApDB::GetPlatformBackUpConfig(const s8* pchPlatformDomainMOID, CBackUpConfig& cBackUpConfig)
{
    TSqlString tSqlString;

	tSqlString.Format("select a."PLATFORM_DOMAIN_DISASTER_BACKUP", a."PLATFORM_DOMAIN_USE_BACKUP_PLATFORM
		", b."BACKUP_PLATFORM_DOMAIN_MOID
		" from "TABLE_PLATFOEM_DOMAIN_INFO" as a, "TABLE_PLATFORM_DOMAIN_DISASTER" as b "
		"where a."PLATFOEM_DOMAIN_MOID" = b."PLATFOEM_DOMAIN_MOID" and a."PLATFOEM_DOMAIN_MOID" = '%s'", pchPlatformDomainMOID);
    u16 wErrId = m_cDBEngine.ExecSql(tSqlString.GetBuf(), TRUE);
    if (SUCCESS_DBENGINE != wErrId)
    {
        DBOprLog( LOG_LVL_WARNING, "[%s]execSql: %s Fail\n", __FUNCTION__, tSqlString.GetBuf());
        return APS_ERR_DB_EXEC_FAIL;
    }
	if (!m_cDBEngine.IsEof())
	{
		s8 szDisasterBackUp[MAXLEN_DB_VALUE + 1] = { 0 };
		m_cDBEngine.GetFieldValue(PLATFORM_DOMAIN_DISASTER_BACKUP, szDisasterBackUp, sizeof(szDisasterBackUp));
		s8 szUseBackUpPlatform[MAXLEN_DB_VALUE + 1] = { 0 };
		m_cDBEngine.GetFieldValue(PLATFORM_DOMAIN_USE_BACKUP_PLATFORM, szUseBackUpPlatform, sizeof(szUseBackUpPlatform));
		s8 szBackUpPlatformMOID[MAXLEN_DB_VALUE + 1] = { 0 };
		m_cDBEngine.GetFieldValue(BACKUP_PLATFORM_DOMAIN_MOID, szBackUpPlatformMOID, sizeof(szBackUpPlatformMOID));
		CBackUpConfig::EMBackUpMode emBackUpMode;
		if (!GetBackUpMode(szDisasterBackUp, szUseBackUpPlatform, emBackUpMode))
		{
			return APS_ERR_REJECT_LOGIN;
		}
		cBackUpConfig.SetBackUpMode(emBackUpMode);
		cBackUpConfig.SetBackUpPlatMOID(szBackUpPlatformMOID);
	}
	else
	{
		cBackUpConfig.SetBackUpMode(CBackUpConfig::EM_BACKUP_CLOSE);
	}
...
u16 CApDB::GetPlatformBriefSrvInfo(const s8* pchPlatformDomainMOID, u16 dwDeviceType, std::set<CBriefSrvInfo>& setBriefSrvInfo)
{
    TSqlString tSqlString;
    tSqlString.Format("SELECT DISTINCT * FROM %s WHERE %s = %d AND %s = ('%s') AND %s = %d",
        TABLE_SERVER_DEVICE_INFO, 
        SRV_DEVICE_USED_FLAG, 1, 
        SRV_DEVICE_PLATFORMDOMAIN_MOID, pchPlatformDomainMOID,
        SRV_DEVICE_SERVER_TYPE, dwDeviceType);
	u16 wErrId = m_cDBEngine.ExecSql(tSqlString.GetBuf(), TRUE);
    if (SUCCESS_DBENGINE != wErrId)
    {
    ...
    while( !m_cDBEngine.IsEof() )
    {
        ...
        if (SUCCESS_DBENGINE != wErrId)
        {
            DBOprLog(LOG_LVL_WARNING, "[%s] wErrId.%u, ReadSrvDeviceInfo fial!\n", __FUNCTION__, wErrId);
            return APS_ERR_DB_EXEC_FAIL;
        }
		CBriefSrvInfo tBriefInfo;
        tBriefInfo = Convert(tInfo);

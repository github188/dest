...
BOOL32 CCenterCtrlData::StarMqMsgParse()
{
    ...
    //获取自己的GUID
	TGUID tMyGUID;
	tMyGUID.SetNOString(g_cCfgFile.GetApsGUID().c_str());
    if (!tMyGUID.IsValid())
    {
...
void CCenterCtrlData::MqMsgParseCB( MqMsgCBEvent emEventID, void* pMsgBody, u32 wMsgLen)
{
	...
	//全面过滤非本平台域设备
	if (strcmp(pSrvDevInfo->GetPlatformDomainMOID(), g_cSrvListMgr.GetPlatformDomainMOID()) != 0)
	{
        CCtrlLog(LOG_LVL_KEYSTATUS, "[%s]bmc server related msg, but <%s>unbelong platform<%s>,ignore\n",
			__FUNCTION__, pSrvDevInfo->GetPlatformDomainMOID(), g_cSrvListMgr.GetPlatformDomainMOID());
		return;
	}
    ...
    case EV_MQMSG_ADD_SERVER_DEVICE:
        {
            ...
            {
				g_cSrvListMgr.AddServerInfo(pSrvDevInfo->GetServerType(), tBriefInfo);
            }
            break;
        }
    ...
    case EV_MQMSG_DISENABLE_SERVER_DEVICE:
        {
			g_cSrvListMgr.DelServerInfo(pSrvDevInfo->GetServerType(), tBriefInfo);
            break;
        }
    case EV_MQMSG_MDF_SERVER_DEVICE:
        {
			g_cSrvListMgr.MdfServerInfo(pSrvDevInfo->GetServerType(), tBriefInfo);
            break;
        }
    default:
        break;

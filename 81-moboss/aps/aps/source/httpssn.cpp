...
static int GetClientIpByXffLastItem(void *cls, enum MHD_ValueKind kind, const char *key, const char *value)
{
	if (strcmp(key, HTTP_COOKIE_CLIENT_IP) == 0)
	{
		s_strClientIp = value;
	}
	return MHD_YES;
...
#endif
{
    ...
    //////////////////////////////////////////cookie分析/////////////////////////////////////////////////////
	s_strClientIp.clear();
	MHD_get_connection_values(connection, MHD_HEADER_KIND, GetClientIpByXffLastItem, NULL);
    if (!s_strClientIp.empty())
    {
        HttpSsnLog(LOG_LVL_WARNING, "client ip:%s \n", s_strClientIp.c_str());
        //已cookie中的IP地址为准(因为Haproxy会篡改源地址)
        client_ip_addr = BossUtils::GetIpByString(s_strClientIp.c_str());
    }
    else
    {
...
{
#ifndef WIN32
    while ( NULL == (m_pHttpsDaemon = MHD_start_daemon (
...
void CHttpSsnInst::ProcUlcLoginAck( CMessage *const pcMsg )
{
	...
	SetApsListToJson(
		tFullInfo.GetSrvInfo().GetMainPlatApsList(),
		tFullInfo.GetSrvInfo().GetApsList(),
		tFullInfo.GetSrvInfo().GetBackUpApsList(),
		g_cCfgFile.GetMtTryApsSpan(), jsonValue);
...
#else
	struct MHD_Response *response = MHD_create_response_from_buffer(wLen, (void *)pbyMsg, MHD_RESPMEM_MUST_COPY);
	if (NULL == response)
	{
		return FALSE;
	}
	MHD_queue_response(handleConnect, MHD_HTTP_OK, response);
	MHD_destroy_response(response);
#endif
    return TRUE;
}

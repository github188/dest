...
class CDmzInfo
{
	...
	u32   m_dwDmzNtpIp;
	u32   m_dwDmzPasIp;
	u32   m_dwDmzRestApiIp;
	u8	  m_byIspType;          //运营商的类型，参照srvmodule_nu.h定义
	s8    m_achOperator[256+ 1];//运营商名称
...
public: 
	...
	void Clear()
	{
		...
		memset( m_achOperator, 0, sizeof( m_achOperator ) );
	}

	//操作符重载
	bool operator == ( const CDmzInfo& tOther ) const
	{
		if ( GetPasIpAddr() == tOther.GetPasIpAddr() == 0)
		{
			return true;
		}
	...
	bool operator < ( const CDmzInfo& other ) const
	{
		if (GetPasIpAddr() < other.GetPasIpAddr())
		{
			return true;
		}
	...
	u32 GetNdsIpAddr( void ) const 
	{
		return ntohl( m_dwDmzNdsIp );
	}
	void SetNdsIpAddr( u32 dwIpAddr )
	{
		m_dwDmzNdsIp = htonl( dwIpAddr );
	}

	u32 GetNmsIpAddr( void ) const 
	{
		return ntohl( m_dwDmzNmsIp );
	}
	void SetNmsIpAddr( u32 dwIpAddr )
	{
		m_dwDmzNmsIp = htonl( dwIpAddr );
	}

	u32 GetSnsIpAddr( void ) const 
	{
		return ntohl( m_dwDmzSnsIp );
	}
	void SetSnsIpAddr( u32 dwIpAddr )
	{
		m_dwDmzSnsIp = htonl( dwIpAddr );
	}

	u32 GetCmcIpAddr( void ) const 
	{
		return ntohl( m_dwDmzCmcIp );
	}
	void SetCmcIpAddr( u32 dwIpAddr )
	{
		m_dwDmzCmcIp = htonl( dwIpAddr );
	}

	u32 GetXmppIpAddr( void ) const 
	{
		return ntohl( m_dwDmzXmppIp );
	}
	void SetXmppIpAddr( u32 dwIpAddr )
	{
		m_dwDmzXmppIp = htonl( dwIpAddr );
	}

	u32 GetVrsIpAddr( void ) const 
	{
		return ntohl( m_dwDmzVrsIp );
	}
	void SetVrsIpAddr( u32 dwIpAddr )
	{
		m_dwDmzVrsIp = htonl( dwIpAddr );
	}

	u32 GetDcsIpAddr( void ) const 
	{
		return ntohl( m_dwDmzDcsIp );
	}
	void SetDcsIpAddr( u32 dwIpAddr )
	{
		m_dwDmzDcsIp = htonl( dwIpAddr );
	}

	u32 GetSusIpAddr( void ) const 
	{
		return ntohl( m_dwDmzSusIp );
	}
	void SetSusIpAddr( u32 dwIpAddr )
	{
		m_dwDmzSusIp = htonl( dwIpAddr );
	}

	u32 GetNtpIpAddr( void ) const 
	{
		return ntohl( m_dwDmzNtpIp );
	}
	void SetNtpIpAddr( u32 dwIpAddr )
	{
		m_dwDmzNtpIp = htonl( dwIpAddr );
	}

	u32 GetPasIpAddr( void ) const 
	{
		return ntohl( m_dwDmzPasIp );
	}
	void SetPasIpAddr( u32 dwIpAddr )
	{
		m_dwDmzPasIp = htonl( dwIpAddr );
	}

	u32 GetRestApiIp( void ) const
	{
		return ntohl( m_dwDmzRestApiIp );
	}

	void SetRestApiIpAddr( u32 dwIpAddr )
	{
		m_dwDmzRestApiIp = htonl( dwIpAddr);
	}

	void SetOperator( const s8* achOperator )
	{
		memset( m_achOperator, 0, sizeof( m_achOperator ) );
		strncpy( m_achOperator, achOperator, sizeof( m_achOperator ) - 1 );	
	}

	const s8* GetOperator( )const
	{
		return m_achOperator;
	} 
